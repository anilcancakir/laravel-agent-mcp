...
Call the tools from the shell with `php artisan agent-mcp:call <tool> '<json>'`, using `agent-mcp:tools` and `agent-mcp:schema` to discover them; they run locally or, with `AGENT_MCP_URL` set, against a remote app. For the full CLI workflow, flags, and per-tool detail, use the `agent-mcp-cli` skill.
@else
